...
export const handleKeyDown = (
  ...
  if (setType === "current" && currentSuggestions.length > 0) {
    if (e.key === "ArrowDown") {
      // Move down through suggestions
      setHighlightedIndex((prev) =>
        prev === null || prev === currentSuggestions.length - 1
          ? 0
          : prev + 1
      );
    } else if (e.key === "ArrowUp") {
      // Move up through suggestions
      setHighlightedIndex((prev) =>
        prev === null || prev === 0 ? currentSuggestions.length - 1 : prev - 1
      );
    } else if (e.key === "Enter" && highlightedIndex !== null) {
      // Set the currently highlighted suggestion when Enter is pressed
      ...
      if (selectedLocation) {
        ...
        setHighlightedIndex(null); // Reset the highlighted index
      }
    }
